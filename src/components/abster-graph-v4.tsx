...
function GraphCanvas({ nodes, edges, selectedNode, selectedEdge, connectMode, connectSource,
  ...
  timelineDate, showAvatars, showClusterBubbles, onSelectNode, onSelectEdge,
        ...
        {nodes.filter((n: any) => !hiddenNodeIds.has(n.id)).map((n: any)=>{
          ...
          return (
            ...
            >
              ...
              {/* runningTransformId indicator removed — LLM hallucination path deleted */}
...
export default function AbsterGraph({ onClose }: { onClose?: () => void }) {
  ...
  const handleRunTransform = useCallback(async (nodeId: string, transformId: string) => {
    ...
    // Every transform in TRANSFORMS has a toolCategory, so this branch
    // always fires and opens the MaltegoSidebar (external-tools catalog).
    // The previous LLM-hallucination path (runTransform in lib/transforms.ts)
    // was removed because it generated fabricated OSINT entities.
    const transform = Object.values(TRANSFORMS).flat().find(t => t.id === transformId);
    if (transform?.toolCategory) {
      setMaltegoTools({ nodeId, transformId, category: transform.toolCategory });
      return;
    }
    // Fallback: if a future transform is added without toolCategory, surface
    // a clear error instead of silently hallucinating data.
    console.warn('Transform has no toolCategory and no LLM fallback:', transformId);
  }, [nodes, activeCaseId]);
